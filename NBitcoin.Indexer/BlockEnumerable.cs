...
namespace NBitcoin.Indexer
{
    public class BlockEnumerable : IEnumerable<StoredBlock>
    {
        ...
        Queue<long> _LastProgresses = new Queue<long>();
        public IEnumerator<StoredBlock> GetEnumerator()
        {
            ...
            long lastLoggedCurrentProgress = 0;
            ...
            foreach (var block in _Store.Enumerate(_Range))
            {
                ...
                {
                    _LastProgresses.Enqueue(_Progress.ProcessedBytes - lastLoggedCurrentProgress);
                    if (_LastProgresses.Count > 50)
                        _LastProgresses.Dequeue();

                    lastLoggedProgress = DateTime.Now;
                    lastLoggedCurrentProgress = _Progress.ProcessedBytes;
                    var speeds = _LastProgresses.OrderBy(o => o).ToArray();
                    IndexerTrace.LogProgress(_Progress, speeds[speeds.Length / 2], _LogInterval);
                }
            }
        }
